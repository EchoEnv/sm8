 ...
 *   before v0.1.0."
 *
 * Per [[scala-impact-analysis-mindset]] §3 ("adding a method to a
 ...
 * implement it. Per [[scala-error-handlingmindset]] §1 ("errors are
 ...
 *
 * Per [[karpathy-app-design-mindset]] §3.1: this is the Protocol for
...
trait TypedRealizationProvider extends EngineProvider {
  ...
  /** Typed realization from a parsed `EngineUrl`. Returns
    ...
    * on failure (typed error per `scala-error-handlingmindset` §1).
    *
...
object TypedRealizationProvider {
  ...
  /** Default `realizeTyped` impl: converts `EngineUrl` → raw string,
    ...
    *
    * Per [[scala-error-handlingmindset]] §1 + §2: this is the IO
    ...
    * granular error.
    *
    * Per [[scala-jvm-safety-mindset]] §2: every resource that opens
