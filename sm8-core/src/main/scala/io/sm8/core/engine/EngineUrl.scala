 ...
 *
 * Per [[scala-data-driven-refactor-mindset]] §3: "default to sealed
 ...
 *
 * Per [[scala-jvm-safety-mindset]] §2 + [[scala-spark-batch-bugs-mindset]]
 ...
 *
 * Per [[scala-data-driven-refactor-mindset]] §1 "data is data": this
...
object EngineUrl {
  ...
  /** Engine-name-only factory: dispatches to the engine-specific parser
    ...
    *
    * Per [[scala-error-handling-mindset]] §1: returns
    * `Either[EngineError, EngineUrl]` (typed error, not silent `None`).
    *
    * Per [[scala-data-driven-refactor-mindset]] §2: validity is enforced
