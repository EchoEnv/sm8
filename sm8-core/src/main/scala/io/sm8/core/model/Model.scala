...
/** Model lifecycle status. Sealed per. */
...
final case class Dimension(
    ...
    dataType: Option[io.sm8.core.schema.SealedDataType] = None) extends Product with Serializable
...
object SourceRef {
  ...
  final case class ByName(
    ...
    table:     String) extends SourceRef
...
/** Typed validation error. Sealed per [[]]. */
...
object ModelValidationError {
  ...
  /** PR-M2: aggregated cross-reference validation errors. All
    * collected errors are surfaced at once (per 
