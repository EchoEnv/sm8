 ...
 * sealed-trait/match over Map tables": HookOrigin is a sealed
 * trait with three cases; the priority-range check is a pure
 * function over (HookOrigin, Int) returning Either[String, Unit]
 * (typed, no exceptions for control flow beyond the boundary).
 *
 ...
 *
 * Per [[scala-impact-analysis-mindset]]: this file is on the SDK
 * side (io.sm8.sdk) so the SDK trait `HookManager` can import it.
 * Plugin authors reach it via the SDK import path
 * (`import io.sm8.sdk.HookOrigin`).
 *
 * Per [[scala-error-handling-mindset]]: the return shape is
 * `Either[String, Unit]` at the validator level. The impl throws
