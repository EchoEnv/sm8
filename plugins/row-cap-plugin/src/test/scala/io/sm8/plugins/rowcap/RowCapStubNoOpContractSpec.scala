...
class RowCapStubNoOpContractSpec extends AnyFunSuite with Matchers {

  // -- Per ADR-008-AC: the hook is a no-op; Context is preserved --

  test("RowCapStub: hook run is a no-op (Context returned unchanged)") {
    val plugin = new RowCapStub(RowCapConfig(maxRows = 1000))
  ...
  test("RowCapStub: hook run increments the fires counter") {
    val plugin = new RowCapStub(RowCapConfig(maxRows = 1000))
    val engine = EngineImpl()
    engine.use(plugin)
    val hook = engine.hooks.postHooksFor(HookStage.PostExecute).head._1
