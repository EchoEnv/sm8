...
class RowCapPluginContractSpec extends HookContractSpec {
  ...
  override def postHook: PostHook = {
    ...
    val plugin = new RowCapPlugin(config)
    engine.use(plugin)
...
class RowCapPluginContractPluginSpec extends PluginContractSpec {

  override def plugin: Plugin = new RowCapPlugin(RowCapConfig(maxRows = 1000))
