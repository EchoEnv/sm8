...
class BroadcastStubNoOpContractSpec extends AnyFunSuite with Matchers {

  // -- Per ADR-008-AC: the hook is a no-op; Context is preserved --

  test("BroadcastStub: hook run writes the broadcast arm meta key + threshold (Context.stop preserved)") {
   // Per ADR-009-d v0.3: the hook's PreExecute.run writes the
   // broadcast arm Boolean + the threshold bytes to context.meta
   // (no SDK change; meta is the standard plugin-to-adapter
   // channel). The Context.stop is NOT mutated (no-op contract on
   // the pipeline's short-circuit flag). The "no real Spark
   // integration" contract holds: the hook never imports Spark
   // and never writes Spark conf.
   ...
   // Non-EngineHookRequest requests are a no-op (the test fixture
   // uses BroadcastConformanceRequest; verify the arm key is NOT
   // written — the hook returns the input unchanged for non-
   // engine-hook requests).
   outputContext.meta shouldBe inputContext.meta
  }

  test("BroadcastStub: hook run increments the fires counter") {
   val plugin = new BroadcastStub()
   val engine = EngineImpl()
   engine.use(plugin)
   val hook = engine.hooks.preHooksFor(HookStage.PreExecute).head._1
  ...
  // estimatedRows <= BroadcastThresholdRows (10M), it ARMS the
  // broadcast seed. These tests drive a real EngineHookRequest +
  // real Model with joins to prove the value-consult is the
  // falsifiable core of the wiring.

  private def hookOf(plugin: BroadcastStub): io.sm8.sdk.PreHook = {
    val engine = EngineImpl()
    engine.use(plugin)
  ...
  test("BroadcastStub hook: model with join estimatedRows <= 10M arms broadcast (sm8.broadcast.arm=true)") {
   // Value-consult arming: BroadcastThresholdRows = 10_000_000.
   // 1_000_000 rows is well below the threshold → arm = true.
   val plugin = new BroadcastStub()
   val hook   = hookOf(plugin)
   val before = plugin.fires.get
   val out = hook.run(baseCtx(engineHookRequestFor(modelWithJoin(Some(1_000_000L)))))
   out.meta.get("sm8.broadcast.arm") shouldBe Some(true: Boolean)
   out.meta.get("sm8.broadcast.thresholdBytes") shouldBe Some(10L * 1024L * 1024L: Long)
   plugin.fires.get shouldBe (before + 1)
  }

  test("BroadcastStub hook: model with join estimatedRows > 10M disarms broadcast (sm8.broadcast.arm=false)") {
   // Value-consult disarming: 100_000_000 rows is well above the
   // 10M threshold → arm = false. This is the disagreement case
   // with the inline presence rule (which would arm any join).
   val plugin = new BroadcastStub()
   val hook   = hookOf(plugin)
   val before = plugin.fires.get
   val out = hook.run(baseCtx(engineHookRequestFor(modelWithJoin(Some(100_000_000L)))))
   out.meta.get("sm8.broadcast.arm") shouldBe Some(false: Boolean)
   out.meta.get("sm8.broadcast.thresholdBytes") shouldBe Some(10L * 1024L * 1024L: Long)
   plugin.fires.get shouldBe (before + 1)
  }

  test("BroadcastStub hook: model with no joins disarms broadcast (sm8.broadcast.arm=false)") {
   // Value-consult disarming: a model with no joins cannot
   // benefit from a broadcast (no small-side exists). arm = false.
   val plugin = new BroadcastStub()
   val hook   = hookOf(plugin)
   val before = plugin.fires.get
