/*
 * SM8 AuditStub Plugin — No-op contract spec (PR-140 per ADR-008-AC).
 *
...
class AuditStubNoOpContractSpec extends AnyFunSuite with Matchers {

  // -- Per ADR-008-AC: the hook is a no-op; Context is preserved --

  test("AuditStub: hook run is a no-op (Context returned unchanged)") {
    ...
    val hook = engine.hooks.postHooksFor(HookStage.PostFormat).head._1
    val outputContext = hook.run(inputContext)
    outputContext shouldBe inputContext
  }

  test("AuditStub: hook run increments the fires counter") {
    val plugin = new AuditStub()
    val engine = EngineImpl()
    engine.use(plugin)
    val hook = engine.hooks.postHooksFor(HookStage.PostFormat).head._1
