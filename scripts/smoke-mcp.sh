...
# (EOF), and assert all 7 tool names appear in the stdout JSON-RPC
# responses.
#
...
# 3. Assert the 7 tool names appear in the stdio response.
for tool in query list_models describe_model list_engines get_metrics list_plugins list_hooks; do
...
echo "SMOKE-MCP PASS (stdio MCP server: 7 tools listed; handshake compliant; serverInfo correct)"