 ...
 * Per [[scala-jvm-safety-mindset]]: this object has no mutable state.

// Bare form with §N:
 * Per [[scala-spark-batch-bugs-mindset]] §2: every assertion is on the evaluated result.
 ...
 * Per [[scala-error-handling-mindset]] §1: errors are data.

// Lowercase "per":
 * per [[scala-perf-testing-mindset]]: warm the JIT before measuring.