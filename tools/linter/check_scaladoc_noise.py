...
NOISE_PATTERNS = [
    ...
    (r"\bper\s+(?:scala-(?!jvm-safety(?=\s|\Z|[.,:§)])|spark-batch-bugs(?=\s|\Z|[.,:§)])|error-handling(?=\s|\Z|[.,:§)])|data-driven-refactor(?=\s|\Z|[.,:§)])|jar-packaging(?=\s|\Z|[.,:§)])|perf-testing(?=\s|\Z|[.,:§)])|2-scaladoc(?=\s|\Z|[.,:§)])|bug-hunting(?=\s|\Z|[.,:§)])|chaos-testing(?=\s|\Z|[.,:§)])|impact-analysis(?=\s|\Z|[.,:§)]))|karphyaguids|mindset-|karpathy-(?!guidelines(?=\s|\Z|[.,:§)])|app-design(?=\s|\Z|[.,:§)]))|debug-mantra-)[\w-]*", "bare skill-citation in .scala source — typo skill name or unknown skill (allowed: scala-jvm-safety, scala-spark-batch-bugs, scala-error-handling, scala-data-driven-refactor, scala-jar-packaging, scala-perf-testing, scala2-scaladoc, scala-bug-hunting, scala-chaos-testing, scala-impact-analysis, karpathy-guidelines, debug-mantra, karpathy-app-design); link to a Scala symbol or external URL instead"),
]
