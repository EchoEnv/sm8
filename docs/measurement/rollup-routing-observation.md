...
| `nonCanonicalShape` | The plan isn't the canonical `Scan → Filter* → Aggregate` shape. **Known causes:** (a) the projection above the Aggregate carries a CALCULATED expression (non-FieldRef, e.g. a computed dim or a derived/calculated measure) — v1 routing requires every projected column to be a plain pass-through of a column the Aggregate produces; models with calculated measures or calculated dimensions will refuse here **until the rewriter learns to fold calculated projections**; (b) the plan genuinely isn't the shape (caller bug) | For (a): either drop the calculated measure/dimension from the model's routing-critical queries, or split it into a separate non-rollup query — v1 cannot route it. For (b): caller-side reshape |
