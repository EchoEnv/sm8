...
| **Status** | **Implemented (PR-189, `5e104cc`) — was Accepted (v0.3), promoted to Implemented on merge** |
...
| v3.2 (Implemented) | 2026-08-28 | Implementation PR-189 (`5e104cc`) merged: 5 atomic commits (`4e3fbb4` HookRunnerOrchestration skeleton + spec; `2e8f12d` EngineService + QueryService migration; `46bb969` v0.3 typed-error surfacing + cycle regression spec; `fe11dc7` graph + audit regression specs; `13d12b4` full reactor re-run). All 7 falsifiable acceptance criteria verified end-to-end: orchestrator drives all 4 stages; `Context.stop = true` short-circuits across stages (per the NEW Verify-advisor point 3 requirement); `execute` thunk only runs at `Execute` stage; cycle model returns typed `Left(EngineError.UnsupportedCapability("SemanticGraph.cycle", ...))`, NOT `ProviderInvocationFailed("NoResult")`; `GraphSnapshot.MetaKey` present in `MetaCaptureObserver`; `AuditStub.fires.get() > 0`; full reactor green (116/116 sm8-platform tests, all other in-scope modules green). Subagent factored the dispatcher into `private[platform] def runStage(stage, initial, execute)` + kept the 2-arg `run` as a delegate (`runStage(PipelineStage.Execute, initial, execute)`) — this is CLEANER than the v0.3 ADR text (which said "preserve the 2-arg `run` as-is" without specifying the factoring). The factoring note is folded here in v3.2 to keep ADR vs implementation honest. 2 P3 nits from parent deep-review folded into the implementation PR (no code changes — just docs alignment): (a) the `runStage` factoring note above; (b) 2-arg `run` still delegates to `runStage(Execute, ...)` — fine but worth a test asserting the delegation path works. Retrospective at `docs/project_status/2026-08-26-adr-010-a-orchestration-layer-retrospective.md`. |
