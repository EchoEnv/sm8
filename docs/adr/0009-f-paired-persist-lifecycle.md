...
| Field | Value |
|---|---|
| **Status** | **Implemented** (PR-180, `821d270`) — was Accepted (v3.6), promoted to Implemented on merge. Decision approved across **6 ADR revisions** (v0.1 → v1.0 → v2.0 → v3.1 → v3.2 → v3.6). 9 implementation commits (`88950db` Core+Platform Fix 6+6b; `08f00c0` Core Fix 4+5; `cd10907` Fix 3 Cache typed-reject; `806b8ef` Spark connector Fix 0+1+2+2b; `5224f38` close() restructure). Dual code review (data-eng best-coding 0.9 + architect best-reasoning 0.95, both APPROVED) folded all findings — 2 data-eng P3s as documented Spark 3.5 trade-offs, 1 architect P2 (close() outer-try logging restructure), 3 architect P3s (stale comment, stale line ref, revision row order), 1 docs tightening (cause-priority falsifiable form vs the infeasible `getSuppressed.length == 1`). Full reactor green: 634/634 core + 241/241 spark-connector + 6 P0/P1 closes (ADR-008-P CROSS-P0-B + 5 gaps). Standing patterns honored: RFC §3 Core Boundary, PR-176 NonFatal discipline, PR-178 honor-or-UnsupportedCapability, PR-179 cap+1 + truncated preservation. v0.1.0 tag still blocked by user's "don't bump version yet" directive. |
| **Date** | 2026-08-26 |
