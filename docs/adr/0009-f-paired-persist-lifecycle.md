...
| Field | Value |
|---|---|
| **Status** | **Accepted (v3.3) — implementation merged on `adr/009-f-paired-persist-lifecycle` (4 atomic commits: 88950db Core+Platform Fix 6+6b, 08f00c0 Core Fix 4+5, cd10907 Fix 3 Cache, 806b8ef Fix 0+1+2+2b on `SparkEngineProvider`); PR-180 open; data-eng final code review **APPROVED 0.9**; architect final code review retry pending (initial dispatch failed on OmniRoute 503 admission-busy after 10 retries). Full reactor green (14 modules, 261 spark-connector tests pass). V3.2 spec corrections honored in 806b8ef (`NonFatal(collectErr)` extractor; `throw collectErr` on the `registerToken == 0L` path) — ADR-009-e non-swallow test green. v3.1 mandates honored (`persistedFramesSize` seam at line 189, Fix 6+6b landed in one commit 88950db, explicit `ThrowingUnpersistDataset` seam at line 702 + `forceUnpersistFault` overload at line 718). v3.3 fold (this revision): data-eng P3 #1 (`addSuppressed` chain — both-fail path — not black-box testable in Spark 3.5 because Project-over-InMemoryRelation returns `storageLevel == NONE`, so a UDF-throwing-and-persisted frame cannot simultaneously keep `registerToken != 0L` and reach the inline `df.unpersist()` fault path; the `addSuppressed` discipline is code-review-verifiable only); data-eng P3 #2 (acceptance #2 test exercises the close sweep via direct `trackPersist` calls — pipeline-registered frames leave the map empty per-query so no test seam is needed for in-flight queries; JVM-shutdown mid-query is the meaningful case and the close path is identical regardless of how tokens arrived). |
| **Date** | 2026-08-26 |
  ...
  val collected: Either[EngineError, Array[org.apache.spark.sql.Row]] =
    try {
      ...
      case NonFatal(collectErr) =>
        // v3.2 correction (pre-implementation review): two refinements.
        // (1) `NonFatal(e)` extractor, not `case e: NonFatal` (NonFatal
        // is an object with unapply, not a type — the v2.0 snippet's
        // `case collectErr: NonFatal` does not compile).
        // (2) The typed Left is returned ONLY when a frame was actually
        // tracked (registerToken != 0L). With registerToken == 0L the
        // collect() failure is NOT a persist-lifecycle failure — it is a
        // plain query failure whose ADR-009-e contract (exception
        // propagates; dispatcher NonFatal-wraps it) is preserved by
        // rethrowing. This keeps PersistLifecycleFailed truthfully
        // labeled (lifecycle-only) and keeps ADR-009-e's existing
        // non-swallow falsifiable test (SparkException thrownBy) green.
        if (registerToken != 0L) {
          // Failure path WITH a tracked frame: unpersist inline (no
          // finally — typed Left returns). Chain any unpersist failure
          // as suppressed so the dispatcher sees the original collect()
          // error first.
          try df.unpersist()
          ...
          // the ADR-009-e non-swallow contract.
          throw collectErr
        }
    }
