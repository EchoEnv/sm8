...
| Field | Value |
| **Status** | **v0.4 — fifth dual review round; v0.3's re-point count was 8 (off by one) and the inheritance note had a self-contradicting parenthetical; v0.4 enumerates all 9 sites explicitly (including `compileModelToDataFrame` `seedBroadcastThreshold` and the explain `private lazy val resolver` with the null-safety guard for the `null-spark` provider); the previous v0.3 corrections (honest inheritance + correct closure-capture) are kept |
| **Date** | 2026-08-24 |
| **Module** | `connectors/spark-connector` (`SparkEngineProvider.query` — per-query `newSession()` forking a fresh `SessionState`; the broadcast seed moves to the per-query session too) + `sm8-core` (`JoinHints.skewFactor` seam, unchanged) |
