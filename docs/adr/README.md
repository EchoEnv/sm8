...
| [0009-f](0009-f-paired-persist-lifecycle.md) | Paired persist lifecycle — typed registration (`trackPersist`/`untrackPersist`), non-swallow unpersist + exception-shadowing chain (Fix 2), `MaterializePolicy.Cache` typed reject, single-source `MaterializePolicy` ADT (drops dead `EngineContext.materializePolicy` + unused `MemoryOnly`/`MemoryAndDisk`/`EngineDefault` cases + 5 test sites in `EngineContextSpec.scala`), typed `EngineError.PersistLifecycleFailed` + `PersistPhase` ADT (replaces v1.0 throw with typed Left return on both paths), 13th case in `QueryService.engineErrorCode` exhaustive match (Fix 6b), close() unpersist loop non-swallow + token-in-log (Fix 2b). Closes ADR-008-P CROSS-P0-B (still OPEN) + 5 sibling gaps surfaced by the v0.3 retrospective + 1 newly-surfaced close() gap. | Accepted (v2.0) |
## Tools
