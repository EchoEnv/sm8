...
| [0010-a](0010-a-enginehookdispatcher-stage-parameter.md) | EngineHookDispatcher stage parameter — lift the hardcoded `PipelineStage.Execute` so non-Execute-stage hooks fire in production (currently 3 first-party plugins silently inert: `JoinPathPreHook` cycle validator, `GraphPostResolveObserver` snapshot writer, `AuditPostStubHook` counter). Decision: thread `stage: PipelineStage` through `dispatcher.run` as a required parameter; callers pass the right stage. Regression tests assert dispatcher-mediated firing for all 3 plugins. | Proposed (v0.1) |
## Tools
