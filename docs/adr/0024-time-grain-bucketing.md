...
**Behavior preservation note**: the four-arm routing partition (§4) preserves every existing observable behavior — `(None, None)` matches (the entire grain-less test surface), `(Some(r), None)` and `(None, Some(q))` refuse via `GrainMismatch` (pinned today at `RollupRewriterSpec`'s "Query no grain, rollup day" assertion), and mismatched grains refuse. What's NEW is only the coarsening YES arm: `(Some(finer), Some(coarser))` for Sum/Count/Min/Max/Avg — a pure capability addition; no existing query loses rollup serving.
