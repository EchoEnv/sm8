...
| Field | Value |
|---|---|
| **Status** | **Proposed (v3.0)** — implementation merged on `adr/009-g-cache-policy-contract` (4 atomic commits: 7f3ffcf core ADT collapse, 8fb0639 core test migration + integration spec, b087dae platform fold, 9eabd41 plugin both-hook gate); PR-182 open. Dual code review APPROVED: architect (best-reasoning) 0.94 + data-eng (best-coding) 0.97. v3.0 docs tightening: 2 P3 prose nits folded in 9eabd41 (stale `WriteThrough: only write; do NOT short-circuit on read` comment at CachePlugin.scala:148 corrected; stale line-number refs in EngineContextSpec.scala:6-10 corrected). The 3rd data-eng P3 (cache-region routing pre-existing gap, not introduced by this PR) is already documented in §Out-of-scope "Multi-region cache routing" — no docs addition needed. Status header reflects implementation + dual review; ADR will be promoted to Implemented via the closeout PR after user authorization.
| **Date** | 2026-08-26 |
