...
| Field | Value |
|---|---|
| **Status** | **Implemented** (PR-182, `8d5e4cb`) — was Accepted (v3.0), promoted to Implemented on merge. Decision approved across **6 ADR revisions** (v0.1 → v1.0 → v1.1 → v3.0; the v0.1/v1.0/v1.1/v3.0 numbering follows the v0.4-wave ADR convention where the data-eng's verdict-and-flagged findings fold into the next revision before merging). 4 implementation commits (`7f3ffcf` core ADT collapse; `8fb0639` core test migration + integration spec; `b087dae` platform fold; `9eabd41` plugin both-hook gate). Dual senior review APPROVED: architect (best-reasoning) 0.94 + data-eng (best-coding) 0.97. Pre-merge author-gated review: cache-plugin 0.95, core 0.95, platform 0.93 — all APPROVED. 3 P3 prose nits folded in `9eabd41` + `8763d0a`. Full reactor green (634/634 core + 241/241 spark-connector + 103/103 platform + 6/6 cli + 33/33 server). 6 pre-existing cache-feature gaps closed (incl1 `CacheReadPreHook` + `CacheWritePostHook` unconditional fire; dual `CachePolicy` ADT drift hazard; dead `EngineContext.cachePolicy` field; fold-site placement; ADR-009-f §Out-of-scope deferral). Standing patterns honored: RFC §3 Core Boundary, PR-176 NonFatal discipline, PR-178 honor-or-UnsupportedCapability, PR-179 cap+1 + truncated preservation, ADR-009-d fold pattern (canonical engine-portable SDK channel). Local-only `.omp/WATCHDOG.yml` (gitignored) tunes the advisor roster for this project.
| **Date** | 2026-08-26 |
...
| v3.0 (Accepted) | 2026-08-26 | Implementation merged on `adr/009-g-cache-policy-contract` (4 atomic commits 7f3ffcf / 8fb0639 / b087dae / 9eabd41). PR-182 open. Dual code review APPROVED: architect (best-reasoning) 0.94 + data-eng (best-coding) 0.97. 2 P3 prose nits folded in 9eabd41: stale `WriteThrough: only write; do NOT short-circuit on read` inline comment at `CachePlugin.scala:148` corrected to "reads on lookup (HITs short-circuit via stop = true, exactly like ReadThrough); the cache-write side-effect lives in CacheWritePostHook (per Fix 6 matrix)"; stale line-number refs in `EngineContextSpec.scala:6-10` (lines "16" / "14-24") removed since the migrated test body is self-describing. The 3rd data-eng P3 (cache-region routing pre-existing gap — `InMemoryResultCache` ignores the `name` field; `ReadThrough("region-a")` and `ReadThrough("region-b")` with identical query shapes share cache entries) is a pre-existing gap not introduced by this PR — already documented in §Out-of-scope "Multi-region cache routing" as a separate ADR candidate; no docs addition needed. Full reactor green (parent re-run confirmed BUILD SUCCESS in 02:03 min after the v3.0 fold). |

| v3.1 (Implemented) | 2026-08-26 | Closeout: promoted to **Implemented** via PR-182 (`8d5e4cb`). Decision approved across 4 numbered revisions (v0.1 → v1.0 → v1.1 → v3.0) + this closeout row. Pre-merge author-gated 3-reviewer code review (cache-plugin 0.95, core 0.95, platform 0.93) APPROVED with 2 P3 prose nits folded in `8763d0a` (indentation drift + double-blank-line in spec files). Local-only `.omp/WATCHDOG.yml` (gitignored) tunes the advisor roster for this project. No follow-ups from this ADR — `Multi-region cache routing` deferred per §Out-of-scope. |
