...
| Field | Value |
|---|---|
| **Status** | **Proposed (v1.0)** — initial draft v0.1 was BLOCKED by architect review (best-reasoning, 0.45 confidence): 5 findings all load-bearing. v1.0 folds all 5: (a) Fix 4 fold-site correction (move from `engineExecutor` to `initialCtx` construction BEFORE `dispatcher.run`; `EngineHookDispatcher.run` takes `initial: Context` and `firePre` runs first — `CacheReadPreHook` is `PreExecute`, fires before `engineExecutor`); (b) Fix 1 extended to gate the post-write hook too (`CacheWritePostHook` is unconditional today — `writeFires` counter increments and `cache.putJournaled` runs on every successful engine result regardless of `model.cache`); (c) Fix 2 ADT undercount corrected (`EngineContext.cachePolicy` typed as `io.sm8.core.engine.CachePolicy` at lines 38 + 59, defaulted at line 59; `EngineContextSpec.scala` lines 14-24 test the engine-side ADT for 4 cases, lines 16-20 + 100 + 109-115 reference the field — these test sites + the spark-connector `PortableQueryCompiler.scala:534` rejection message must be enumerated + migrated to `io.sm8.core.model.CachePolicy`); (d) Fix 6 explicit per-case policy matrix (resolves the ReadThrough-on-miss contradiction); (e) Fix 5 dispatcher API surface documented (the `initial: Context` parameter is the seam; no dispatcher API change needed — the fold goes into `initialCtx.meta` construction at `EngineService.scala:387-393`). |
| **Date** | 2026-08-26 |
...
| `EngineContextSpec.scala:109-115` (EngineContext holds-all-5-fields test) | UPDATE field reference to `io.sm8.core.model.CachePolicy.NoCache` |
...
4. EngineContext.cachePolicy fold target: a model with
   ...
   dispatcher integration spec: build a hook request with
   model.cache = ReadThrough("region-a"), run through the
   full dispatcher, observe the hook received the right
   value in ctx.meta.
