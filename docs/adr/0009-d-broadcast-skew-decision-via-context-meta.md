...
| Field | Value |
|---|---|
| **Status** | **Proposed v0.3 (rebuilt after v0.2 was BLOCKED by dual review — see Revision history v0.1 → v0.2 → v0.3)** |
| **Date** | 2026-08-25 |
...
12. **The decision lives only at PreExecute.** We do not add the decision to `PostExecute` or other stages. PreExecute is when the model is bound and the seed helpers have not yet run. Other stages are out of scope for this ADR.
